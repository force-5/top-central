...
class EmployeePraFileFeedService extends FileFeedService {
    ...
    EmployeePraFileFeedService() {
        feedName = CareConstants.FEED_EMPLOYEE_PRA
        fileName = ConfigurationHolder.config.feed.employeePraFileFeed.fileName
    ...
    List getVOs(def rows) {
        ...
        // Get only the Employee PRA rows
        rows = rows.findAll { (it?.split('\\|')?.toList()*.trim()?.get(5)?.toString()?.equalsIgnoreCase('EMPLOYEE PRA')) }
    ...
    def process(Object feedVO) {
        try {
            ...
            WorkerPraCertificationInfo workerPraCertificationInfo = praFeedVO.toEmployeePraCertificationInfo().s()
            FeedRunReportMessageDetail messageDetail = new FeedRunReportMessageDetail(entityType: "WorkerPraCertificationInfo", param1: workerPraCertificationInfo.slid ?: workerPraCertificationInfo.pernr, param2: workerPraCertificationInfo.lastBackgroundDate, entityId: workerPraCertificationInfo.id)
